...
class klineCreator:
    ...
    def write_f(self,tag,kline):
        df = self.dataframe(kline)
        df.to_csv("data/{}.csv".format(tag))
    ...
    def run(self, **kwargs):
        symbols = ["bsv","dot","eth","btc","sushi","ht","yfi","yfii","ksm","ltc","uni","kava","qtum","xrp","fil","doge","crv","iost","xlm","usdc","btc3s","eth3s"]
        for symbol in symbols:
            symbol_usdt = "{}_usdt".format(symbol) 
            self.task.register(self.found_kline, 5,period="1min" ,size=self.data_lenght, symbol=symbol_usdt, callback=huspm,**kwargs)
