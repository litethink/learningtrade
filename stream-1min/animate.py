...
def animate(i):
    ...
    y1 = derepeat(data1.values)
    y2 = derepeat(data2.values)
    if len(y1) == len(y2):
        ...
        p1 = np.polynomial.Chebyshev.fit(x1, y1, 9)
        y_fit1 = p1(x1)

        p2 = np.polynomial.Chebyshev.fit(x2, y2, 9)
        y_fit2 = p2(x2)

        p3 = np.polynomial.Chebyshev.fit(x3, y3, 9)
        y_fit3 = p3(x3)
        ...
        golden = plot_golden()
        plt.plot(y1, "r", label='btc spot')
        plt.plot(y2, "g", label='eth swap')
        plt.plot(y3, "y", label="mean two")
        plt.plot(y_fit1, "k", label="fit btc")
        plt.plot(y_fit2, "c", label="fit eth")
        plt.plot(y_fit3, "m", label="fit mean")
